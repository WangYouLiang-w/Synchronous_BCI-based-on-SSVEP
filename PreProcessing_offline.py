...
class PreProcessing():
    ...
    def __init__(self,filepath, t_begin, t_end, n_classes=40, fs_down=1000, chans=None, num_filter=1):
        '''
        init()
    ...
    def load_cnt_data(self):
        ...
        '''
        raw = mne.io.read_raw_cnt(self.filepath, eog=['HEO', 'VEO'], emg=['EMG'], ecg=['EKG'], preload=True, verbose=False)
        num = 255
        mapping_ssvep = dict()
        ...
        Nc = len(self.chans)
        nTimes = int(stimlength * self.fs_down)
        ...
        raw_data = np.zeros((Nc, nTimes, Nblocks, Ntarget))
        for trigger_i in range(uniquetrigger.shape[0]):
            ...
            for j in range(currenttriggerpos.shape[0]):
                raw_data[:, :, j, uniquetrigger[trigger_i]-1] = data[:, int(currenttriggerpos[j]+self.t_begin*self.fs_down+2):int(currenttriggerpos[j]+self.t_end*self.fs_down+2)]*1000000
    ...
    def load_edf_data(self):
        eeg = readbdfdata.readbdfdata(self.filepath)
        data = eeg['data']
        events_ssvep = eeg['events']
        stimlength = self.t_end - self.t_begin
        Nc = len(self.chans)
        nTimes = int(stimlength * self.fs_down)
        ...
        for trigger_i in range(uniquetrigger.shape[0]):
            ...
            currenttriggerpos = triggerpos[np.where(triggertype==currenttrigger)]
            for j in range(currenttriggerpos.shape[0]):
                raw_data[:, :, j, uniquetrigger[trigger_i]-1] = data[:, int(currenttriggerpos[j]+self.t_begin*self.fs_down):int(currenttriggerpos[j]+self.t_end*self.fs_down)]*1000000
    ...
    def load_benchmark_data(self):
        ...
        if isinstance(self.chans, list):
            for _, char_value in enumerate(self.chans):
                idx_loc.append(self.CHANNELS.index(char_value.upper())) #.upper() 将字符串中的小写转成大写

        raw_data = raw_data[idx_loc, int(self.t_begin*self.fs_down):int(self.t_end*self.fs_down), :, :] if idx_loc else raw_data
        self.raw_fs = 250
        return raw_data
    

    def load_beta_data(self):
        '''
        Application: load data and selected channels by chans.
    ...
    def filtered_data_iir(self, w_pass_2d, w_stop_2d, data):
        ...
        filtered_data = np.zeros((w_pass_2d.shape[1], data.shape[0], data.shape[1], data.shape[2], data.shape[3]))
        for idx_filter in range(self.num_filter):
            ...
            # 前后滤波
            data1 = np.transpose(data,[1,0,2,3])
            ...
            filtered_data[idx_filter,:, :, :, :] = np.transpose(filter_data, [1,0,2,3])
