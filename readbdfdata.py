...
def readbdfdata(pathname):
    ...
    raw = mne.io.read_raw_bdf(os.path.join(pathname,'data.bdf'), preload=False)
    ...
    ## read events
    try:
        annotationData = mne.io.read_raw_bdf(os.path.join(pathname,'evt.bdf'))
        try:
