...
log_file_name = subject_name + '_R_BL'+'_simonline'
...
if __name__ == '__main__':
    ...
    dataRunner = algorithmthread(fs=srate, w=w, template=template, addloop=OVERLAP_TRIALS, Nfb=1, mode='R_LB')
    ...
    # test:
    Task_count = 0
    coef_mat = np.zeros((nclasses-1, OVERLAP_TRIALS))
    Task_acc = []
    rrall = np.zeros((nclasses-1, 1))
    control_mat = np.zeros((nclasses, nclasses))
    Task_mat  = np.zeros((nclasses, nclasses))
